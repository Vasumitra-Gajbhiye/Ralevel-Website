...
function readMetadata(claims: SessionClaims): ClerkPublicMetadata | undefined {
...
function readUserDataIdFromClaims(claims: SessionClaims): string | undefined {
  ...
  const metadata = readMetadata(claims);
  return typeof metadata?.userDataId === "string" &&
    metadata.userDataId.length > 0
    ? metadata.userDataId
    : undefined;
}

function readPublicMetadata(metadata: ClerkPublicMetadata | undefined): {
  roles: Role[];
