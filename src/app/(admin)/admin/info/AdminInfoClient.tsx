...
import type { AdminInfoMember } from "@/lib/data/admin/info";
...
function InfoCard({
  ...
  function update(patch: Partial<AdminInfoMember>) {
    setData((p) => ({ ...p, ...patch }));
    setSaving("saving");
    ...
    >
      ...
      <div className="grid grid-cols-[1.4fr_1.4fr_1.8fr_1.2fr_0.6fr] gap-4 items-center">
        ...
        <select
          ...
          onClick={(e) => e.stopPropagation()}
          onChange={(e) =>
            update({ activity: e.target.value as AdminInfoMember["activity"] })
          }
          className={`border rounded px-3 py-2 ${
...
export default function AdminInfoClient({
  initialMembers,
  pagination,
}: {
  initialMembers: AdminInfoMember[];
  ...
  async function addMember() {
    await fetch("/api/admin/info", { method: "POST" });
    router.refresh();
  }
  ...
  return (
    <div className="max-w-7xl mx-auto p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Community Department</h1>
      ...
      <ListPagination
        ...
        onPageChange={(nextPage) => router.push(`/admin/info?page=${nextPage}`)}
      />
    </div>
  );
}
