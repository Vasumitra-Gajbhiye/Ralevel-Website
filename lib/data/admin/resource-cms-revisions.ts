...
function serializeRevisionListItem(doc: RevisionDoc): ResourceCmsRevisionListItem {
...
export async function restoreResourceCMSRevision(params: {
  ...
  const revision = await ResourceCmsRevision.findById(params.revisionId).lean<
    RevisionDoc | null
  >();
  ...
  const { getResourceCMSDocSnapshot, resolveCMSDraft } = await import(
    "./resource-cms"
  );
