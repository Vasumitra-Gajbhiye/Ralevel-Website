...
function extractListItems(children: ReactNode): ReactNode[] {
  ...
  const getLiChildren = (element: React.ReactElement): ReactNode[] => {
    const props = element.props as { children?: ReactNode };
    const liNodes = React.Children.toArray(props.children);
    const items: ReactNode[] = [];
    for (const node of liNodes) {
      if (isValidElement(node)) {
        const nodeProps = node.props as { children?: ReactNode };
        if (nodeProps.children != null) items.push(nodeProps.children);
      }
    }
    return items;
  };
