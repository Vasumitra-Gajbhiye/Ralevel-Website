...
module.exports = {
  ...
  async execute(interaction) {
    ...
    // create collector for this message
    // Note: in djs v14, componentType should usually be ComponentType.Button (number 2),
    // but the string works depending on your exact minor version.
    const collector = replyMsg.createMessageComponentCollector({
      componentType: 2, // 2 is Button
      time: COLLECTOR_TIMEOUT,
    });

    collector.on("collect", async (btnInt) => {
      // ✂️ REPLACED CRASHING MOD_ROLES CHECK WITH EXECUTOR CHECK
